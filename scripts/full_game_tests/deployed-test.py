...
async def create_test_agent_subprocess(agent_id: str, server_url: str):
    """Create a test agent as a subprocess (allows logs to show)."""
    print(f"🤖 Starting agent subprocess: {agent_id}")
    ...
    try:
        ...
        print(f"✅ Agent {agent_id} subprocess started (PID: {process.pid})")
        return process
...
async def main():
    """Run the production test with 4 local agents."""
    ...
    print("🎮 Production Test: 4 Local Agents")
    print("=" * 50)
    ...
    try:
        ...
        # Create 4 test agents as subprocesses
        agent_names = ["alice", "bob", "charlie", "diana"]
        
        print("🤖 Starting agent subprocesses...")
        for agent_name in agent_names:
            process = await create_test_agent_subprocess(agent_name, server_url)
            if process:
